...
Do NOT skip history analysis. Instead, execute this full fallback protocol automatically. It reconstructs the same information using zero-cost sources.

#### Step 3a: Map periods to date ranges

Convert each `discovery_only` period string to concrete dates for Wayback Machine targeting:
...
Only run the fallback for periods that include a high-signal change type: `Price Increased`, `Price Decreased`, `Plan Added`, `Plan Removed`, `Plan Renamed`, `Discount Removed`, `Discount Added`. Skip periods with only `Feature Added` or `Feature Changed` — those don't require diff reconstruction.

#### Step 3b: Run Wayback Machine snapshot fetches

For each high-signal period (run all in parallel):

```
# 1. Find available snapshots bracketing the change:
WebFetch(url="https://archive.org/wayback/available?url={domain}/pricing&timestamp={before_date}")
WebFetch(url="https://archive.org/wayback/available?url={domain}/pricing&timestamp={after_date}")
```

From each response, extract the `closest.url` field. Then fetch both snapshots:

```
WebFetch(url="{closest_url_before}")
WebFetch(url="{closest_url_after}")
```

Diff the text content of the two fetched pages:
- Plan names that appeared, disappeared, or were renamed
- Price values that changed (scan for `$` followed by numbers)
- Feature descriptions that were added to or removed from specific tiers
- CTA or trial language changes

#### Step 3c: Run parallel web fallbacks

For each high-signal period, run all of the following in parallel (alongside Step 3b):

```
# Third-party price trackers — often cache historical pricing:
...
# Targeted announcement search using the period date range:
WebSearch(query='"{Company}" pricing {change_type} {period_year} "{period_month_range}"')
# e.g. for 2025Q4: '"Linear" pricing increase 2025 "October" OR "November" OR "December"'

# Community discussion — customers often post price increase emails or screenshots:
WebSearch(query='"{Company}" price increase {period_year} site:reddit.com OR site:news.ycombinator.com')
WebSearch(query='"{Company}" pricing change {period_year} announcement OR email OR notification')

# Google Cache — may still show old pricing for recent changes:
WebSearch(query='cache:{domain}/pricing')

# Changelog — companies often frame price changes via blog posts:
WebFetch(url="{domain}/changelog")
WebSearch(query='"{Company}" pricing changelog {period_year}')
```

#### Step 3d: Run visual-diff workflow

For each high-signal period reconstructed, run [visual-diff.md](visual-diff.md) to capture and embed the before/after screenshots. This runs in parallel with Step 3c — do not wait for text reconstruction to complete first.

#### Step 3e: Synthesise and output reconstructed diff

Present findings in the output under a clearly labelled section:

```markdown
#### {Period} — {N} changes detected by PricingSaaS (reconstructed without credits)

*PricingSaaS flagged {N} change(s) in this period: {change types}. Full diff costs {N} credits. The following is reconstructed from Wayback Machine snapshots, third-party trackers, and community sources.*

##### Reconstructed change: {Change type}
**Before ({before_date} snapshot):** {What the pricing page showed — plan names, prices, limits}
**After ({after_date} snapshot):** {What changed}
**Delta:** {Specific before → after, e.g., "$8/user/mo → $10/user/mo"}

[View before snapshot →]({wayback_before_url})
[View after snapshot →]({wayback_after_url})
[View on PricingSaaS (requires credits) →](https://pricingsaas.com/pulse/companies/{slug}/diffs/{period})

**Sources used:** {list which fallbacks returned useful data — Wayback Machine / SaasPricePulse / Reddit / changelog / etc.}
```

If Wayback Machine has no snapshots for a period AND web sources found nothing specific, note:
> "No credit-free reconstruction possible for {period}. Change type was '{type}' — unlock with {N} credit(s) when available."

Do not leave a period silently blank. Always document what was attempted.
